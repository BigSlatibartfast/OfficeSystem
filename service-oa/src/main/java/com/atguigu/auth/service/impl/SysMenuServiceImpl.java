...
public class SysMenuServiceImpl extends ServiceImpl<SysMenuMapper, SysMenu> implements SysMenuService {
    ...
    public List<SysMenu> findMenuByRoleId(Long roleId) {
        ...
        //2.根据角色id roleId查询 角色菜单关系表里 角色id对应所有的菜单id
        LambdaQueryWrapper<SysRoleMenu> wrapperSysRoleMenu = new LambdaQueryWrapper<>();
        wrapperSysRoleMenu.eq(SysRoleMenu::getRoleId,roleId);
        List<SysRoleMenu> sysRoleMenuList = sysRoleMenuService.list(wrapperSysRoleMenu);

        //3.根据获取的菜单id，获取 对应菜单对象
        List<Long> menuIdList = sysRoleMenuList.stream().map(c->c.getMenuId()).collect(Collectors.toList());

        //3.1 拿菜单id和所有菜单集合里面id进行比较，如果相同封装
    ...
    public void doAssign(AssginMenuVo assginMenuVo) {
        ...
        //2.从参数里边获取角色新分配菜单id列表，进行遍历，把每个id数据添加菜单角色表
    ...
    private List<RouterVo> buildRouter(List<SysMenu> menus) {
        ...
        for (SysMenu menu : menus) {
            ...
            if(menu.getType().intValue() == 1){
                //加载出来下面隐藏路由
                List<SysMenu> hiddenMenuList = children.stream().filter(item -> !StringUtils.isEmpty(item
                        .getComponent())).collect(Collectors.toList());
    ...
    public List<String> findUserPermsByUserId(Long userId) {
        ...
        //3.从查询出来的数据里边，获取可以操作按钮值的list集合，返回
